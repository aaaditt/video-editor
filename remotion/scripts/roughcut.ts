 ...
 * cut segments out of the source, apply speed changes, concatenate.
 *
...
if (!fs.existsSync(planPath)) {
  console.error(`No edit plan at ${planPath}`);
  process.exit(1);
}

const plan = editPlanSchema.parse(JSON.parse(fs.readFileSync(planPath, "utf8")));
...
for (const seg of plan.segments) {
  if (seg.end <= seg.start) {
    console.error(`Invalid segment: end (${seg.end}) <= start (${seg.start})`);
    process.exit(1);
  }
...
const cutSegment = (seg: Segment, index: number): string => {
  const outFile = path.join(jobDir, `seg-${String(index).padStart(3, "0")}.mp4`);
  const vf = `${scaleFilter},setpts=PTS/${seg.speed},fps=${fps}`;

  const args = ["-y", "-hide_banner", "-loglevel", "warning", "-ss", String(seg.start), "-to", String(seg.end), "-i", source];
  args.push("-vf", vf);
...
execFileSync(
  ffmpegPath,
  ["-y", "-hide_banner", "-loglevel", "warning", "-f", "concat", "-safe", "0", "-i", "concat.txt", "-c", "copy", "roughcut.mp4"],
